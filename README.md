...
**Views and commands.** A **view** is per-user config layered over a project: a named "bundle shape" of include/exclude deltas over the project's repo set, so different people can start bundles with different subsets of the same project (`knit view save`, `knit view default`). A project can also register reusable commands that run inside a bundle's checkouts (`knit project command set dev --repo frontend -- docker compose up`, then `knit run dev`). Projects with a `runtime` block additionally get `knit run up|status|down`: Knit injects a per-bundle environment contract (`KNIT_CHECKOUT_<repo>`, `KNIT_REV_<repo>`, allocated ports, database identity) into a compose file owned by the stack repo and runs it as an isolated compose project per bundle. Both are covered in full in the reference.
