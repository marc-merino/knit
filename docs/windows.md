...
**Status: untested / in progress.** Knit has been developed and run only on
...
| External forge CLIs | `src/providers/github.rs:14` (`gh`), `src/providers/gitlab.rs:9` (`glab`), `src/providers/forgejo.rs:9` (`tea`) | Medium | Spawned via `Command::new(bin)` (`src/providers/mod.rs:404`). `std`'s `Command` resolves `.exe` but **not** `.cmd`/`.bat`/`.ps1` shims. If a forge CLI is installed only as a `.cmd` shim (some installers do this), spawning fails with "program not found". Document as a known quirk; do not refactor blindly. |
...
| Worktree/context bundle resolution | `src/store.rs:234-242` (`infer_worktree_bundle`), `src/store.rs:440-456` (`resolve_context_bundle`) | Medium | `strip_prefix` / `starts_with` on raw `PathBuf`s are case-sensitive component compares. On Windows a cwd that differs only by drive-letter or directory casing from the recorded path will fail to resolve the bundle. Not fixed here (needs a careful normalization story shared with `same_path`). |
...
| Config dir resolution | `src/store.rs:122-133` (`global_config_path`) | Medium | Resolves `KNIT_HOME` → `XDG_CONFIG_HOME` → `$HOME/.config/knit`. Windows has no `HOME` by default (`USERPROFILE`/`%APPDATA%` instead). On Windows, `KNIT_HOME` must be set explicitly or global config will error with "No home directory found". A proper fix would consult `dirs`/`known-folders` for `%APPDATA%`. Documented as a blocker for global config without `KNIT_HOME`. |
...
Recommendation to revisit: **(A)+(B)** — a forward-slash, relative-where-possible
